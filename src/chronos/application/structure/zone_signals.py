...
detector. Donde no hay ID propio —M15 y M5— tampoco hay señales propias.
...
2. y esa señal se **re-fecha** en la vela de la serie más corta de la corrida
   —M5 en el reparto del proyecto— en la que el precio entró en la zona, con los
...
def _finest_chart(run: ImpulseRun) -> tuple[str, CandleSeries] | None:
    ...
    Es uno de los gráficos que la corrida ya construyó —M5 en el reparto del
    proyecto—, no un histórico aparte. Si el reparto no llevara ninguno más fino
