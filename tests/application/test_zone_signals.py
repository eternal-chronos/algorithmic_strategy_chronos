...
def test_solo_hay_senales_donde_hay_detector(signals: ZoneSignalsRun) -> None:
    """En el reparto por defecto H1, M15 y M5 no llevan ID propio: ni zonas ni señales."""
...
def test_el_toque_del_ob_se_fecha_en_la_vela_fina(
    ...
    """No se espera al cierre de H4: el toque va en la vela de M5 que lo hizo.

    El rechazo y la rotura del UL sí necesitan un cierre y se quedan en la
    rejilla de la temporalidad del ID.
    """
    finas = pd.DatetimeIndex(run.chart_bars["M5"].index)
